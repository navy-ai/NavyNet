...
class Epoch:
    ...
    def __init__(self, model, loss, metrics, stage_name, device='cpu', verbose=True, scale=512, is_train=True):
        self.model = model
        self.loss = loss
    ...
    def run(self, dataloader):
        ...
        with tqdm(dataloader, desc=self.stage_name, file=sys.stdout, disable=not (self.verbose)) as iterator:
            ...
            for (x1, x2, y, filename) in iterator:
                ...
                wandb_dict = {}
                wandb_dict[mode + '_' + self.loss.__name__]=loss_meter.mean
            for k,v in metrics_meters.items():
                wandb_dict[mode+'_'+str(k)] = float(v.mean)
            # print(wandb_dict)
...
class TrainEpoch(Epoch):
    ...
    def __init__(self, model, loss, metrics, optimizer, device='cpu', verbose=True, extra_pre=False, accumulation=False, scale=512, is_train=True):
        super().__init__(
            model=model,
            loss=loss,
            metrics=metrics,
...
class ValidEpoch(Epoch):
    ...
    def __init__(self, model, loss, metrics, device='cpu', verbose=True, extra_pre=False, TTA=False,is_train='False'):
        super().__init__(
            model=model,
            loss=loss,
            metrics=metrics,
